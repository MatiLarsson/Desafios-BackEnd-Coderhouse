...
// Al conectarse un usuario en la vista current, le manda a hacer fetch de los productos y de su carrito, actualizados.
app.set('socketio', io)
...
io.on('connection', async (socket) => {
  ...
  socket.emit('log', normalize({ id: 1, chats: await chatsService.getAllChats() }, chat)) // Reemplazamos con el metodo de mongoose y normalizamos los chats
  socket.broadcast.emit('newUser', socket.id)
  socket.on('message', async (message) => {
    await chatsService.saveChat(message) // Reemplazamos con el metodo de mongoose
    io.emit('log', normalize({ id: 1, chats: await chatsService.getAllChats() }, chat)) // Reemplazamos con el metodo de mongoose y normalizamos los chats
  })
})
...
=> implementar las vistas de fail para register y login. (Tiene mas que ver con como passport entrega los errores).
...
=> Separar arquitectura por capas.
